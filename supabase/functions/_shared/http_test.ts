...
Deno.test('safeErrorCode does not expose message contents', () => {
  assertEquals(safeErrorCode(new TypeError('child data must never be logged')), 'TypeError');
});

Deno.test('invitation HTML escapes untrusted household labels', () => {
  assertEquals(escapeHtml('<script>"Village" & friends</script>'), '&lt;script&gt;&quot;Village&quot; &amp; friends&lt;/script&gt;');
});
